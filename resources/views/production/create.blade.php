            ...
            <div class="col-lg-12 col-md-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center bg-transparent border-bottom-0 mt-3">
                        <div class="col-6">
                            <h5 class="m-0">Create Production ({{ ucfirst($type) }})</h5>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="basic-form">
                            {{ html()->form('POST', route('production.save', $type))->attribute('id', 'frmProduction')->open() }}
                            <div class="row">
                                <div class="mb-3 col-md-2">
                                    <label class="form-label req">Production Date</label>
                                    {{ html()->date('date', old('date') ?? date('Y-m-d'))->class("form-control")->required() }}
                                    @error('date')
                                ...
                                <div class="mb-3 col-md-4">
                                    ...
                                    {{ html()->select($name = 'from_company_id', $companies, old('from_company_id'))->class('form-control')->required() }}
                            ...
                            <div class="row">
                                <div class="mb-3 col-md-2">
                                    ...
                                    {{ html()->select('items[]', $materials->where('id', $item->id)->pluck('name', 'id'), $item->id)->class('form-control') }}
                                    @error('items')
                                ...
                                <div class="mb-3 col-md-2">
                                    ...
                                    {{ html()->number('qty[]', '', '', '', 'any')->class("form-control")->placeholder("0.0")->required() }}
                                    @error('qty')
                            ...
                            <div class="row">
                                <div class="col text-end">
                                    ...
                                    {{ html()->submit("Save Production")->attribute('onClick', "return checkInventory('frmProduction', '$type')")->class("btn btn-submit btn-primary") }}
                                </div>
                            </div>
                            {{ html()->form()->close() }}
                        </div>
